...
async def test_search_without_cache(
    ...
    expected_pagination_result = set(
        [row[movies_settings.es_id_field] for row in paginate_list(
    ...
    generated_films.extend(generate_films(
        num_films=30, film_title="It's a Wonderful Life"))
    ...
    response_pagination_result = set([row['uuid']
                                      for row in response_body['films']])
