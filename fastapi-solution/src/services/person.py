...
class PersonService:
    ...
    async def _get_persons_by_name_from_elastic(
        ...
        """Returns persons by name from Elasticsearch"""
        ...
        persons = [
            Person.parse_obj(x["_source"]) for x in docs["hits"]["hits"]
        ]
        return persons
    ...
    async def _persons_by_key_from_cache(
        ...
        """Returns persons by name from cache"""
    ...
    async def _put_person_by_key_to_cache(
        ...
        await self.redis.hset(
            "person_key", key, orjson.dumps(persons, default=dict)
        )
    ...
    async def _get_person_films_from_elastic(
        ...
        async for doc in async_scan(
            client=self.elastic, index="movies", query=query
        ):
            docs.append(doc)
    ...
    async def _person_data_from_cache(
        ...
        """Returns person films by id from cache"""
    ...
    async def _put_person_data_to_cache(
        self, person_id: str, person_data: list[Film]
    ):
        """Save a person data to cache (uses redis hset)"""
